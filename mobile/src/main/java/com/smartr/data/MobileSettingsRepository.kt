...
data class WatchSettings(
    ...
    val pingValue: Int = 1,
    val pingUnit: TimeIntervalUnit = TimeIntervalUnit.HOURS
)
...
class MobileSettingsRepository(private val context: Context) {
    companion object {
        ...
        private val PING_VALUE = intPreferencesKey("ping_value")
        private val PING_UNIT = intPreferencesKey("ping_unit")

        val DEFAULTS = WatchSettings(45, TimeIntervalUnit.MINUTES, 20, TimeIntervalUnit.MINUTES, 22, 6, false, 1, TimeIntervalUnit.HOURS)
    }
    ...
    val watchSettings: Flow<WatchSettings> = context.dataStore.data.map { prefs ->
        WatchSettings(
            ...
            pingValue = prefs[PING_VALUE] ?: DEFAULTS.pingValue,
            pingUnit = TimeIntervalUnit.entries[prefs[PING_UNIT] ?: DEFAULTS.pingUnit.ordinal]
        )
    }
    ...
    suspend fun updatePingFrequency(value: Int, unit: TimeIntervalUnit) {
        context.dataStore.edit { prefs ->
            prefs[PING_VALUE] = value
            prefs[PING_UNIT] = unit.ordinal
        }
    }
